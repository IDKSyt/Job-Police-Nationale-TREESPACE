--[[
---------------------------------------------------
Contrôle des sirènes V3 (FOR FIVEM)
---------------------------------------------------
Coded by Lt.Caine
...
RMenu.Add('lvc', 'main', RageUI.CreateMenu("Menu Sirene", "Main Menu"))
RMenu.Add('lvc', 'maintone', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "Paramètres principaux de la sirène"))
RMenu.Add('lvc', 'hudsettings', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "HUD Paramètres"))
RMenu.Add('lvc', 'audiosettings', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "Paramètres Audio"))
RMenu.Add('lvc', 'plugins', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "Plugins"))
RMenu.Add('lvc', 'saveload', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "Gestion du stockage"))
RMenu.Add('lvc', 'copyprofile', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "Copier les paramètres du profil"))
RMenu.Add('lvc', 'about', RageUI.CreateSubMenu(RMenu:Get('lvc', 'main'),"Contrôle des sirènes", "À propos de Contrôle des sirènes"))
...
Keys.Register(open_menu_key, open_menu_key, 'LVC: Open Menu', function()
	if not key_lock and player_is_emerg_driver and UpdateOnscreenKeyboard() ~= 0 and settings_init then
		if UTIL:GetVehicleProfileName() == "DEFAULT" then
			...
			sl_btn_debug_msg = "Utilisation du profil ~b~DEFAULT~s~ pour \"~b~" .. veh_name .. "~s~\"."
		else
			sl_btn_debug_msg = ""
		end
...
Citizen.CreateThread(function()
    while true do
	    ...
	    RageUI.IsVisible(RMenu:Get('lvc', 'main'), function()
			RageUI.Separator("Paramètres de la sirène")
			RageUI.Button('Paramètres principaux de la sirène', "Modifiez quelle/comment chaque tonalité principale disponible est utilisée.", {RightLabel = "→→→"}, true, {
			...
			if custom_manual_tones_master_switch then
				...
				RageUI.List('Tonalité manuelle principale', TonesTable, PMANU-1, "Modifiez votre tonalité manuelle principale.", {}, true, {
				  onListChange = function(Index, Item)
					UTIL:SetToneByID('PMANU', Item.Value)
				  end,
				  onSelected = function()
					proposed_name = HUD:KeyboardInput("Entrez le nouveau nom de la tonalité pour" .. SIRENS[PMANU].String .. ":", SIRENS[PMANU].Name, 15)
				...
				RageUI.List('Tonalité manuelle secondaire', TonesTable, SMANU-1, "Changez votre tonalité manuelle secondaire.", {}, true, {
				  onListChange = function(Index, Item)
					UTIL:SetToneByID('SMANU', Item.Value)
				  end,
				  onSelected = function()
					proposed_name = HUD:KeyboardInput("Entrez le nouveau nom de la tonalité pour" .. SIRENS[SMANU].String .. ":", SIRENS[SMANU].Name, 15)
					if proposed_name ~= nil then
			...
			if custom_aux_tones_master_switch then
				...
				RageUI.List('Tonalité de sirène auxiliaire', TonesTable, AUX-1, "Modifiez la tonalité de votre sirène auxiliaire/double.", {}, true, {
				  onListChange = function(Index, Item)
					UTIL:SetToneByID('AUX', Item.Value)
				  end,
				  onSelected = function()
					proposed_name = HUD:KeyboardInput("Entrez le nouveau nom de la tonalité pour " .. SIRENS[AUX].String .. ":", SIRENS[AUX].Name, 15)
					if proposed_name ~= nil then
			...
			if park_kill_masterswitch then
				RageUI.Checkbox('Sirènes Park Kill', "Bascule si vos sirènes s'éteignent automatiquement lorsque vous quittez votre véhicule. ", park_kill, {}, {
			...
			RageUI.Separator("Autres réglages")
			RageUI.Button('Paramètres de l\'HUD', "Ouvrez le menu des paramètres de l'HUD.", {RightLabel = "→→→"}, true, {
			  onSelected = function()
			  end,
			...
			RageUI.Button('More Information', "Learn more about Contrôle des sirènes.", {RightLabel = "→→→"}, true, {
			  onSelected = function()
			  end,
			}, RMenu:Get('lvc', 'about'))
        end)
						...
						RageUI.List(SIRENS[tone].Name, { 'Cycle & Button', 'Cycle Only', 'Button Only', 'Disabled' }, UTIL:GetToneOption(tone), "~g~Cycle:~s~ play as you cycle through sirens.\n~g~Button:~s~ play when registered key is pressed.\n~b~Select to rename siren tones.", {}, true, {
							onListChange = function(Index, Item)
								if UTIL:IsOkayToDisable() or Index < 3 then
									...
									HUD:ShowNotification("~y~~h~Info:~h~ ~s~Contrôle des sirènes\nAction prohibited, cannot disable all sirens.", true) 
								end
							end,
							onSelected = function()
